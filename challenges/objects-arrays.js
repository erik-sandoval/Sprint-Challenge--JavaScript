...
for (let index = 0; index < graduates.length; index++) {
  contactInfo.push(graduates[index]['first_name']);
  contactInfo.push(graduates[index]['email']);
  contactInfo.push(',')
...
for (let index = 0; index < graduates.length; index++) {
  if (universities[index].includes('Uni') === true) {
...
const newMap = zooAnimals.map(function (element) {
  return element.population;
})

const reducer = newMap.reduce(function (element, populationTotal) {
  return populationTotal + element;
})
